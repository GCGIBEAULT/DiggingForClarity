...
// Load your lookup maps via require (must be next to this file)
...
async function getHeadlines(city, feedMap) {
  ...
  // If fallback to “default,” suppress any purely commercial feeds
  if (city === "default") {
  ...
  // Aggregate all RSS items from each feed URL
  let allItems = [];
  ...
  // Pick the 10 newest, shape them, then filter clickbait, all-caps, exclaims, very short
  const baitWords = [
